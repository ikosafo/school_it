...
<?php }
    ...
    $main_query = $mysqli->query("select * from student where status is null and class = '$stud_class'"); ?>
    ...
    <?php  while ($get_this = $main_query->fetch_assoc()) {
        ...
        $query_id = $mysqli->query("select * from student where status is null and student_id = '$student_id'");
...
AND grade >= '$elective_grade_1' AND id <> '$id'");
    ...
    <table id="" class="table table-bordered table-hover" width="100%" style="margin-top: -18px">
        ...
        while ($row_type = $chk_type->fetch_assoc()) {
            ...
            <?php
            $que = $mysqli->query("SELECT DISTINCT(s.period),s.`course`,s.`class_score`,s.`exam_score`,c.`type`,s.`total`,
s.`remark`,s.`grade`,s.`grade_display`
